...
/*The following code were modified from the dear imgui examples
 * https://github.com/ocornut/imgui/blob/master/examples/imgui_impl_sdl.cpp
 */
#pragma region ImGui SDL Implementation
...
static void ImGui_ImplSDL2_NewFrame(SDL_Window* window)
{
    ImGuiIO& io = ImGui::GetIO();
...
int SRSCALL SR_WM_InitEventSystem(SR_WM_display* display)
{
    ...
    if(!ImGui_ImplSDL2_Init(display->win))
    {
        return -1;
    }
